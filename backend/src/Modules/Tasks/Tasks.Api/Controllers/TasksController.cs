...
public sealed class TasksController(ISender sender) : ControllerBase
{
    ...
    /// <summary>The task as a PDF report, in English or Arabic (<c>?language=ar</c>).</summary>
    [HttpGet("{id:guid}/export-pdf")]
    [Authorize(Policy = NwfmPolicies.ViewTasks)]
    [ProducesResponseType(typeof(FileContentResult), StatusCodes.Status200OK, TaskPdfDto.ContentType)]
    public async Task<IActionResult> ExportPdf(Guid id, [FromQuery] string? language, CancellationToken ct)
    {
        var result = await sender.Send(new ExportTaskPdfQuery(id, language), ct);
        if (!result.IsSuccess)
        {
            return TasksActionResults.Failure(result.Error);
        }

        return File(result.Value.Content, TaskPdfDto.ContentType, result.Value.FileName);
    }
    ...
    [HttpGet("{id:guid}/c2m/logs")]
    [Authorize(Policy = NwfmPolicies.ViewTasks)]
    [ProducesResponseType(typeof(Result<IReadOnlyList<C2mDispatchLogDto>>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetC2mLogs(Guid id, CancellationToken ct) =>
        (await sender.Send(new GetTaskC2mLogsQuery(id), ct)).ToActionResult();

    /// <summary>Sends an approved task's refused or failed C2M closure again, now.</summary>
    [HttpPost("{id:guid}/c2m/retry")]
    [Authorize(Policy = NwfmPolicies.ReviewTasks)]
    [ProducesResponseType(typeof(Result<C2mRetryResultDto>), StatusCodes.Status200OK)]
    public async Task<IActionResult> RetryC2m(Guid id, CancellationToken ct) =>
        (await sender.Send(new RetryTaskC2mClosureCommand(id), ct)).ToActionResult();
