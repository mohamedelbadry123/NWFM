...
public sealed record WorkspaceTask(WorkItemDto Task, bool CanAct, string? DisabledReason);
public sealed record WorkspaceActivity(Guid Id, string NodeKey, string Name, string Type, string Status, string? Phase,
    DateTime StartedAt, DateTime? CompletedAt, DateTime? DueAt, string? DepartmentCode, string? FieldActivityCode, IReadOnlyList<WorkspaceTask> Tasks, string? AssignedGroup = null);
...
public sealed record WorkspaceOperation(Guid Id, Guid InstanceId, Guid ActivityId, string? Name, string Kind, string? Trigger, bool Required, string Status, int Attempts, string? Error, int? StatusCode, string? ResponseJson);
