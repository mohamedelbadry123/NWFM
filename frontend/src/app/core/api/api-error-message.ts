 ...
 *
 * A translated message wins when one exists for the error code (`error.<code>`), so a refusal can
 * read properly in Arabic. Otherwise the server's own message is used: it names the
 * field or the record, which is more useful than a generic failure.
 */
